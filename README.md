...
## PROJECT STRUCTURE 📂
The project is organized logically to separate concerns and ensure a smooth data flow:

- **`app.py`**: The application entry point and factory.
- **`blueprints/`**: Contains the route definitions for different modules:
    - `main.py`: Home, About, and Contact pages.
    - `auth.py`: User authentication, registration, and dashboard.
    - `prediction.py`: Core prediction logic (Crop, Fertilizer, Disease).
    - `api.py`: Backend API endpoints for weather, location, and translation.
    - `admin.py`: Administrative dashboard and functions.
- **`utils/`**: Core business logic and services:
    ...
    - `location_service.py`: Indian location data management.
    - `i18n.py`: Multilingual support and translations.
    - `db_models.py`: Database schemas.
- **`models/`**: Stores trained model files (`.pkl`, `.pth`).
- **`Data/`**: Contains dataset files and CSV data.
